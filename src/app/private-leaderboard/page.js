...
import { PRIVATE_LEADERBOARD_CONFIG } from '@/config/privateLeaderboard'
...
function PrivateLeaderboard() {
  ...
  // Sorting function (same as main leaderboard)
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch('/api/private-leaderboard', {
        method: 'POST',
        headers: {
      ...
      sessionStorage.setItem('privateAccess', password)
      
      setIsAuthenticated(true)
      loadLeaderboardData(jsonData)
  ...
  useEffect(() => {
    const storedPassword = sessionStorage.getItem('privateAccess')
    if (storedPassword) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-2xl p-8 max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Private Leaderboard</h1>
            <p className="text-gray-600 text-sm">
              {PRIVATE_LEADERBOARD_CONFIG.description}
            </p>
          </div>
      ...
      <nav className='w-full shadow-md relative'>
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white w-full m-auto text-center p-2 flex justify-between items-center px-4">
          <div className="flex items-center gap-2">
            ...
            <p className='font-semibold'>🔒 {PRIVATE_LEADERBOARD_CONFIG.title}</p>
          </div>
          <button
        ...
        <div className="sec m-auto my-10 space-y-8 w-1/2 mob:w-full flex flex-col">
          <div className="info flex mob:flex-col mob:justify-center mob:items-center mob:space-y-10 mob:p-5 justify-evenly space-x-3 mob:space-x-0">
            ...
            <div className="eligibleforswag w-fit mob:w-full h-20 p-5 space-x-5 rounded-lg flex flex-row justify-evenly mob:justify-between items-center bg-purple-50 shadow-lg shadow-purple-300/30 border border-purple-200">
              <p className="text-center mob:text-start text-sm text-purple-400">
                Private Participants
              </p>
              <p className="no text-2xl border-l-2 border-l-purple-700 pl-3 text-purple-800">
                {data.length}
              </p>
            </div>
          </div>
